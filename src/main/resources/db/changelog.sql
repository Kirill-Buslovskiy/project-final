...
constraint FK_ACTIVITY_USERS,
    add constraint FK_ACTIVITY_USERS foreign key (AUTHOR_ID) references USERS (ID) on delete
cascade;

alter table USER_BELONG
drop
constraint FK_USER_BELONG,
    add constraint FK_USER_BELONG foreign key (USER_ID) references USERS (ID) on delete
cascade;

alter table ATTACHMENT
drop
constraint FK_ATTACHMENT,
    add constraint FK_ATTACHMENT foreign key (USER_ID) references USERS (ID) on delete
cascade;
...
values (1, 1, '2026-07-18 10:00:00', 'in_progress'),
       (1, 1, '2026-07-18 14:00:00', 'ready_for_review'),
       (1, 1, '2026-07-18 16:00:00', 'done');
